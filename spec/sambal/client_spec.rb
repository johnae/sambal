...
describe Sambal::Client do
  ...
  describe 'ls' do
    ...
    it "should list files using a wildcard on an smb server" do
      ...
      expect(result).to have_key(testfile)
      expect(result).not_to have_key(testfile2)
      expect(result).to have_key(testfile3)
    end
  end
  ...
  it "should get files from an smb server" do
    expect(@sambal_client.get(testfile, "/tmp/sambal_spec_testfile.txt")).to be_success
    expect(File.exists?("/tmp/sambal_spec_testfile.txt")).to be(true)
  ...
  it "should get files in a dir with spaces in it's name from an smb server" do
    expect(@sambal_client.get(test_spaces_in_name_path, "/tmp/sambal_this_file_was_in_dir_with_spaces.txt")).to be_success
    expect(File.exists?("/tmp/sambal_this_file_was_in_dir_with_spaces.txt")).to be(true)
  ...
  it "should get files in a subdirectory while in a higher level directory from an smb server" do
    expect(@sambal_client.get(testfile_sub_path, "/tmp/sambal_spec_testfile_sub.txt")).to be_success
    expect(File.exists?("/tmp/sambal_spec_testfile_sub.txt")).to be(true)
  ...
  it "should not be successful when getting a file from an smb server fails" do
    ...
    expect(File.exists?("/tmp/sambal_spec_non_existant_file.txt")).to be(false)
  end

  it "should upload files to an smb server" do
    expect(@sambal_client.ls).not_to have_key("uploaded_file.txt")
    expect(@sambal_client.put(file_to_upload.path, 'uploaded_file.txt')).to be_success
    expect(@sambal_client.ls).to have_key("uploaded_file.txt")
  end

  it "should upload content to an smb server" do
    expect(@sambal_client.ls).not_to have_key("content_uploaded_file.txt")
    expect(@sambal_client.put_content("Content upload", 'content_uploaded_file.txt')).to be_success
    expect(@sambal_client.ls).to have_key("content_uploaded_file.txt")
  end

  it "should delete files on an smb server" do
    expect(@sambal_client.del(testfile)).to be_success
    expect(@sambal_client.ls).not_to have_key(testfile)
  end
  ...
  it "should switch directory on an smb server" do
    expect(@sambal_client.put_content("testing directories", 'dirtest.txt')).to be_success ## a bit stupid, but now we can check that this isn't listed when we switch dirs
    expect(@sambal_client.ls).to have_key('dirtest.txt')
    expect(@sambal_client.cd(test_directory)).to be_success
    expect(@sambal_client.ls).not_to have_key('dirtest.txt')
    expect(@sambal_client.put_content("in #{test_directory}", 'intestdir.txt')).to be_success
    expect(@sambal_client.ls).to have_key('intestdir.txt')
    expect(@sambal_client.cd('..')).to be_success
    expect(@sambal_client.ls).not_to have_key('intestdir.txt')
    expect(@sambal_client.ls).to have_key('dirtest.txt')
  end
  ...
  it "should delete files in subdirectory while in a higher level directory" do
    ...
    expect(@sambal_client.put_content("some content", "file_to_delete")).to be_success
    @sambal_client.cd('/')
    expect(@sambal_client.del("#{test_directory}/file_to_delete")).to be_success
    @sambal_client.cd('/')
  ...
  it "should recursively delete a directory" do
    ...
    expect(@sambal_client.put_content("some content", "file_to_delete")).to be_success
    @sambal_client.cd('/')
    expect(@sambal_client.rmdir("#{test_directory}")).to be_success
    @sambal_client.cd('/')
    expect(@sambal_client.ls).not_to have_key("#{test_directory}")
  end

  it "should not be successful when recursively deleting a nonexistant directory" do
    @sambal_client.cd('/')
    expect(@sambal_client.rmdir("this_doesnt_exist")).not_to be_success
  end
